    ...
    <div className='bg-white rounded-2xl shadow-lg border border-gray-100 mb-8'>
      <div className='p-6'>
        ...
        <div className='relative mb-6'>
          ...
          <input
            type='text'
            placeholder={isAdmin ? t('filters.search') : t('filters.searchCatalog')}
            value={searchQuery}
        ...
        <div className='flex items-center justify-between mb-4'>
          ...
          <div className='flex items-center space-x-3'>
            <label className='text-sm font-medium text-gray-700'>{t('filters.show')}</label>
            <select
              value={itemsPerPage}
          ...
          <div className='border-t border-gray-200 pt-6 animate-slide-up'>
            {isAdmin ? (
              ...
              <div className='space-y-4'>
                <div>
                  <label className='block text-sm font-medium text-gray-700 mb-2'>{t('filters.productVisibility')}</label>
                  <div className='flex flex-wrap gap-2'>
                    {[
                      { value: 'all', label: t('filters.allProducts') },
                      { value: 'visible', label: t('filters.visibleOnly') },
                      { value: 'non-visible', label: t('filters.hiddenOnly') }
                    ].map((option) => (
                      <button
                        key={option.value}
              ...
              <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4'>
                ...
                {/* Unit Filter */}
                <div>
                  <label className='block text-sm font-medium text-gray-700 mb-2'>{t('filters.unit')}</label>
                  <select
                    name='unidad'
                ...
                {/* Model Filter */}
                <div>
                  <label className='block text-sm font-medium text-gray-700 mb-2'>{t('filters.model')}</label>
                  <select
                    name='modelo'
